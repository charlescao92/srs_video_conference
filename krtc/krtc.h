...
public:
    ...
    virtual void OnPushNetworkInfo(uint64_t rtt_ms, uint64_t packets_lost, double fraction_lost) {}
...
class KRTC_API KRTCMsgObserver {
public:
    // <display, publishing>
    virtual void OnParcicipantsInfo(const std::map<std::string, bool> &) = 0;
    virtual void OnParcicipantJoin(const std::string&, bool publishing) = 0;
    virtual void OnParcicipantLeave(const std::string&, bool publishing) = 0;
    
};

class KRTC_API KRTCEngine {
public:
    static void Init(KRTCEngineObserver* media_observer, KRTCMsgObserver* msg_observer);
    ...
    static bool JoinRoom(const std::string& server_addr,
