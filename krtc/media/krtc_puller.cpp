...
KRTCPuller::KRTCPuller(const std::string& server_addr, const std::string& pull_channel, int hwnd)
    :current_thread_(std::make_unique<KRTCThread>(rtc::Thread::Current()))
{
    pull_impl_ = new rtc::RefCountedObject<KRTCPullImpl>(server_addr, pull_channel, hwnd);
}
