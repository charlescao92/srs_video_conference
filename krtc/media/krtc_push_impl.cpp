...
void KRTCPushImpl::Start() {
    ...
    if (!add_audio_track_result.ok() && !add_video_track_result.ok()) {
        PushFailure(KRTCError::kAddTrackErr);
        return;
    }
...
void KRTCPushImpl::OnSuccess(webrtc::SessionDescriptionInterface* desc) {
    ...
    Json::Value reqMsg;
    reqMsg["api"] = httpRequestUrl_; // "https://1.14.148.67:443/rtc/v1/publish/"
    reqMsg["streamurl"] = webrtcStreamUrl_; // "webrtc://1.14.148.67/55555/1c2582f"
    reqMsg["sdp"] = sdpOffer;
...
void KRTCPushImpl::OnFailure(webrtc::RTCError error) {
    PushFailure(KRTCError::kCreateOfferErr);
}
    ...
    for (auto it = report->begin(); it != report->end(); ++it) {
        ...
        if (type == "outbound-rtp") {
            ...
            if (KRTCGlobal::Instance()->engine_observer()) {
                KRTCGlobal::Instance()->engine_observer()->OnPushNetworkInfo(rtt_ms, packetsLost, fractionLost);
            }
        }
    }
}
...
void KRTCPushImpl::handleHttpPushResponse(const HttpReply &reply) {
    if (reply.get_status_code() != 200 || reply.get_errno() != 0) {
        ...
        PushFailure(KRTCError::kSendOfferErr);
        return;
    }
    ...
    if (!err.empty()) {
        ...
        PushFailure(KRTCError::kParseAnswerErr);
        return;
    }
    ...
    if (code != 0) {
        ...
        PushFailure(KRTCError::kAnswerResponseErr);
        return;
    }
    ...
    // 这里需要发送pushAction消息到ws服务器
    KRTCClient::Instance()->SendPublishMsg();
...
void KRTCPushImpl::PushFailure(const KRTCError& err)
{
    if (KRTCGlobal::Instance()->engine_observer()) {
        KRTCGlobal::Instance()->engine_observer()->OnPushFailed(err);
    }
}
